...
import com.cc.domain.Cart;
import com.cc.domain.CartItem;
...
import com.cc.utils.JedisPoolUtils;
import com.cc.utils.PaymentUtil;
...
public class ProductServlet extends BaseServlet {
	...
	public void confirmOrder(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//直接入一个接口,这个接口已经集成所有的银行接口了，这个接口是第三方支付平台提供的
		//接入的是易宝支付
	...
	public void submitOrder(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//4.private int state; 		//订单支付状态 1代表已支付，0代表未支付
		order.setState(0);
		//5.private String address; //收货地址
		order.setAddress(null);
		//6.private String name;	//收货人
		order.setName(null);
		//7.private String telphone;//收货人电话
	...
	public void addProductToCart(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//获得购物车---判断是否在session中是否存在购物车
		Cart cart=(Cart) session.getAttribute("cart");
		if(cart == null){
			cart = new Cart();
		}
		//将购物项放入车中---key是pid
		//先判断购物车中是否已经将包含此购物项了   判断key是否已经存在
		//如果购物车中已经存在这商品----将现在买的数量与原有的熟练进行相加操作
		Map<String, CartItem> cartItems=cart.getCartItems();
		...
		if(cartItems.containsKey(pid)){
			...
			//修改小计
			//原先该商品的小计
			oldsubtotal= cartItem.getSubtotal();
			//新买的商品的小计
